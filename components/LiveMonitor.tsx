...
const LiveMonitor: React.FC = () => {
  ...
  return (
    <div className="w-full h-full bg-black relative overflow-hidden flex flex-col items-center justify-center font-mono">
                  ...
                  <tbody className="divide-y divide-slate-800 text-slate-300">
                    {selectedEvent.transactions.map((txn) => (
                      <tr key={txn.id} className="hover:bg-rose-900/10 transition-colors">
                        ...
                        <td
                          className="p-3 text-white cursor-pointer hover:text-indigo-400 hover:underline transition-colors font-bold"
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedEntity(txn.sender);
                          }}
                        >
                          {txn.sender}
                        </td>
      ...
      {/* 5. Entity Detail View (Drill Down) */}
      {selectedEntity && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-slate-900 w-full max-w-md border border-indigo-500/50 shadow-[0_0_50px_rgba(99,102,241,0.25)] relative overflow-hidden flex flex-col">

            {/* Header */}
            <div className="bg-indigo-950/50 p-5 border-b border-indigo-500/30 flex justify-between items-center relative overflow-hidden">
              <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 contrast-150"></div>
              <div className="relative z-10">
                <div className="text-[10px] text-indigo-400 font-mono uppercase tracking-widest mb-1">Target Identified</div>
                <h2 className="text-2xl font-black text-white uppercase tracking-tighter">{selectedEntity}</h2>
              </div>
              <button onClick={() => setSelectedEntity(null)} className="relative z-10 text-indigo-300 hover:text-white transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start gap-4">
                <div className="w-20 h-20 bg-slate-800 rounded border border-slate-700 flex items-center justify-center text-slate-600 font-bold text-2xl">
                  {selectedEntity.slice(0, 2).toUpperCase()}
                </div>
                <div className="flex-1 space-y-2">
                  <div className="flex justify-between border-b border-slate-800 pb-1">
                    <span className="text-xs text-slate-500 uppercase">Risk Score</span>
                    <span className="text-xs text-rose-500 font-bold">CRITICAL (98.2)</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-1">
                    <span className="text-xs text-slate-500 uppercase">KYC Status</span>
                    <span className="text-xs text-amber-500 font-bold">SUSPICIOUS</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-1">
                    <span className="text-xs text-slate-500 uppercase">Location</span>
                    <span className="text-xs text-slate-300">Cayman Islands</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-black/40 p-3 rounded border border-slate-800">
                <div className="text-[10px] text-slate-500 uppercase mb-2">Transaction Velocity (24h)</div>
                <div className="flex items-end gap-1 h-16 w-full px-1">
                  {[40, 65, 30, 80, 50, 90, 20, 45, 70, 60, 85, 40].map((h, i) => (
                    <div key={i} style={{ height: `${h}%` }} className="flex-1 bg-indigo-500/50 hover:bg-indigo-400 transition-colors rounded-sm"></div>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold uppercase tracking-wider text-xs rounded transition-colors flex items-center justify-center gap-2"
              >
                <Activity size={14} /> Initiate Deep Scan
              </button>
            </div>
          </div>
        </div>
